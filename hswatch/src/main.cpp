...
TaskHandle_t bt_task_h, timer_task_h, b_left_up_task_h, b_left_down_task_h,
	b_right_up_task_h, b_right_down_task_h;
...
unsigned long debounce_left_up = 0, debounce_left_down = 0,
			  debounce_right_up = 0, debounce_right_down = 0;
...
void setup() {
	...
	*queue_display = xQueueCreate(15, sizeof(msg_queue_display));
	...
	xTaskCreate(b_left_up_task, "button left up task", 8192, NULL, 1,
	            &b_left_up_task_h);
	xTaskCreate(b_left_down_task, "button left down task", 8192, NULL, 1,
	            &b_left_down_task_h);
	xTaskCreate(b_right_up_task, "button right up task", 8192, NULL, 1,
	            &b_right_up_task_h);
	xTaskCreate(b_right_down_task, "button right down task", 8192, NULL, 1,
	            &b_right_down_task_h);
...
int init_display() {
	...
	screen.clear();
	screen.drawXbm((128 - LOGO_WIDTH) / 2, (64 - LOGO_HEIGHT) / 2, LOGO_WIDTH,
	               LOGO_HEIGHT, logo_hs);
	screen.display();
	delay(3000);
