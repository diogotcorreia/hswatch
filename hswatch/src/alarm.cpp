...
void Alarm::timer_1s() {
	...
	if (!(timer_timestamp.hour == t.hour &&
	      timer_timestamp.minute == t.minute)) {
		timer_timestamp.hour = t.hour;
		timer_timestamp.minute = t.minute;

		if (alarm1 && (App::curr_app() != this ||
		               (state != alarm1_hours && state != alarm1_minutes))) {
			if (alarm1_timestamp.hour == t.hour &&
			    alarm1_timestamp.minute == t.minute) {
				alarm1_ringing = true;
				state = alarm1_home;
		...
		if (alarm2 && (App::curr_app() != this ||
		               (state != alarm2_hours && state != alarm2_minutes))) {
			if (alarm2_timestamp.hour == t.hour &&
			    alarm2_timestamp.minute == t.minute) {
				alarm2_ringing = true;
				state = alarm2_home;
		...
		if (alarm3 && (App::curr_app() != this ||
		               (state != alarm3_hours && state != alarm3_minutes))) {
			if (alarm3_timestamp.hour == t.hour &&
			    alarm3_timestamp.minute == t.minute) {
				alarm3_ringing = true;
				state = alarm3_home;
		...
		if (enable_alarm) {
			...
			buzz(buzzer_pattern_power, buzzer_pattern_time,
			     ...
			     buzzer_pattern_repeat, &alarm_buzzer_task);
...
Alarm::Alarm(String id_in, String name_in, const unsigned char* logo_in)
