...
void Menu::display() {
	...
	if (n_app == 0) {
		...
		if (index != 0) {
			Display::drawXbm(0, 16, LOGO_APP_WIDTH, LOGO_APP_HEIGHT,
			                 app_array[index - 1]->logo);
		}

		Display::drawXbm(48, 16, LOGO_APP_WIDTH, LOGO_APP_HEIGHT,
		                 app_array[index]->logo);
		...
		if (index != n_app - 1) {
			Display::drawXbm(95, 16, LOGO_APP_WIDTH, LOGO_APP_HEIGHT,
			                 app_array[index + 1]->logo);
		}
	}
...
Menu::Menu(String id_in, String name_in, const unsigned char* logo_in)
	: App(id_in, name_in, logo_in) {
	int j = 0;
	n_app = 0;

	for (std::list<App*>::iterator i = App::app_list_show().begin();
	     i != App::app_list_show().end(); i++) {
		if ((*i)->logo != NULL) {
			n_app++;
		}
	}
