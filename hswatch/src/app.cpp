...
App::App(String id_in, String name_in, const unsigned char* logo_in)
	: id(id_in), name(name_in), logo(logo_in) {
...
App* App::app_search_by_name(String name_in) {
	for (std::list<App*>::iterator it = app_list.begin(); it != app_list.end();
	     it++) {
		if ((*it)->name == name_in) {
			return *it;
		}
...
void App::call_timer() {
	xSemaphoreTake(mutex_app, portMAX_DELAY);
	for (std::list<App*>::iterator it = timer_attached_app.begin();
	     it != timer_attached_app.end(); it++) {
		(*it)->timer_1s();
	}
	xSemaphoreGive(mutex_app);
}
