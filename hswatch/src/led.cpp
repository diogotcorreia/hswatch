...
void rainbow_led(TaskHandle_t* task_h, int priority) {
	if (!approve_blink(priority)) return;
...
void fade_led(unsigned char r, unsigned char g, unsigned char b,
              TaskHandle_t* task_h, int priority) {
	if (!approve_blink(priority)) return;
...
void fade2_led(unsigned char r, unsigned char g, unsigned char b,
               TaskHandle_t* task_h, int priority) {
	if (!approve_blink(priority)) return;
...
void fade3_led(unsigned char r, unsigned char g, unsigned char b,
               TaskHandle_t* task_h, int priority) {
	if (!approve_blink(priority)) return;
...
void turnon_led(unsigned char r, unsigned char g, unsigned char b,
                TaskHandle_t* task_h, int priority) {
	if (!approve_blink(priority)) return;
...
void create_blink_task(led_pattern* p, TaskHandle_t* task_h, int priority) {
	...
	if (priority == 1) {
		...
		if (led_task != NULL) cancel_blink_led(*led_task);
